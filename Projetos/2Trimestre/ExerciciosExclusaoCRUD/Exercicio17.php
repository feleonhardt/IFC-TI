...
<body>
  <form method="post">
    ...
    <div class="radio radio-inline">
      <input id="3" type="radio" name="radio" value="numero_praticantes" <?php echo $radio == 'numero_praticantes' ? 'checked ' : ' '; ?>/>
      <label for="3">Número de Praticantes</label>
    </div>
    </center>
    <div class="input">
    ...
    <table>
      ...
      <?php 
        if ($radio == 'codigo') {
          ...
          $sql = 'select * from '. $tb_esporte. ' where '. $radio.' like "'. $pesquisa. '%" order by '.$radio;
        } elseif ($radio == 'numero_praticantes') {
          if ($pesquisa == '') {
            $pesquisa = '';
          } else {
            $pesquisa = ' where '. $radio.' = '. $pesquisa. ' order by '.$radio;
        ...
        while ($row = mysqli_fetch_array($resultado)) {
          ...
          echo '<td><a href="exclusao.php?acao=excluir&codigo='.$row[0]. '&tabela='. $tb_esporte. '&url=Exercicio17">Excluir</a></td></tr>';
          echo '</tr>';
        }
      ?>
    </table>
