  ...
  for ($i=0; $i < count($json) ; $i++) {
    if ($i == $altera) {
      echo "<tr>";
        ...
        echo "<td><input type='text' name='novo_nome' value='".$json[$i]->nome."' class='validate' required></td>";
        echo "<td><input type='text' name='novo_telefone' value='".$json[$i]->telefone."' class='validate' required></td>";
        echo "<td><input type='email' name='novo_email' value='".$json[$i]->email."' class='validate' required></td>";
...
function meses(){
  // $json = obtemValorDoJson();
  // for ($i=0; $i < count($json); $i++) {
  //   $date = $json[$i]->nascimento;
  //   $date = explode('/',$date);
  //   $mes = $date[1];
  //   // $dominio = array();
  //   if (!in_array($mes, $vet_meses)) {
  //     $vet_meses[] = (int)$mes;
  //   }
  // }
  //   sort($vet_meses);
